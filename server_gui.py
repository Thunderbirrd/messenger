...
def gui_create_model(database):
    ...
    list1 = QStandardItemModel()
    list1.setHorizontalHeaderLabels(['Имя Клиента', 'IP Адрес', 'Порт', 'Время подключения'])
    for row in list_users:
        ...
        list1.appendRow([user, ip, port, time])
    return list1
...
def create_stat_model(database):
    ...
    list1 = QStandardItemModel()
    list1.setHorizontalHeaderLabels(
    ...
    for row in hist_list:
        ...
        list1.appendRow([user, last_seen, sent, recvd])
    return list1
